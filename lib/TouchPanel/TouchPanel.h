...
public:
  ...
  // Diagnostics for the web tuning page.
  TouchState state() const     { return fsm_.state(); }
  uint16_t   touchedMask() const;
  uint16_t   filtered(uint8_t chan) { return mpr_.filteredData(chan); }
  uint16_t   baseline(uint8_t chan) { return mpr_.baselineData(chan); }
